...
# Use a simpler model that doesn't require TensorFlow
MODEL_NAME = "all-MiniLM-L6-v2"  # This model is lighter and more compatible
...
def hybrid_search_simple(query, k=5, bm25_weight=0.3):
    """Simplified hybrid search that works without complex embeddings"""
    try:
        if not docs:
        ...
        print(f"🔍 Recherche hybride simple: '{query[:50]}...' (k={k})")
        logger.info(f"🔍 Recherche hybride simple: '{query[:50]}...' (k={k})")
        ...
        if model is not None:
            ...
            print("⚠️ Using BM25 only (no embeddings)")
        ...
        print(f"❌ Erreur dans hybrid_search_simple: {str(e)}")
        logger.error(f"❌ Erreur dans hybrid_search_simple: {str(e)}")
        raise Exception(f"Erreur dans hybrid_search_simple: {str(e)}")


# Alias for compatibility
hybrid_search = hybrid_search_simple
