...
latency_hist = Histogram("rag_latency_seconds", "RAG response latency")
retrieval_hist = Histogram("rag_retrieval_docs", "Docs retrieved per query", buckets=[1,2,3,5,10])
ollama_calls = Counter("ollama_calls_total", "Total Ollama API calls")
...
def generate(question: str, evaluate: bool = False, k: int = 5) -> str:
    ...
    try:
        with mlflow.start_run():
            ...
            logger.info(f"✓ {num_docs} documents trouvés")
            
            # Log to MLflow
            mlflow.log_param("query", question[:100])  # Truncate for readability
            mlflow.log_param("k", k)
            mlflow.log_metric("num_docs_retrieved", num_docs)
            
            # Log to Prometheus
            retrieval_hist.observe(num_docs)
