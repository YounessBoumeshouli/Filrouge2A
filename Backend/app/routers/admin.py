...
async def get_analytics_overview(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Total scans
        location_scans = db.execute(text("SELECT COUNT(*) FROM location_scan_history")).scalar()
        price_scans = db.execute(text("SELECT COUNT(*) FROM price_scan_purchase")).scalar()
        ...
        overview["avg_location_rating"] = float(avg_location_rating) if avg_location_rating else None
        overview["avg_price_rating"] = float(avg_price_rating) if avg_price_rating else None
...
async def get_popular_places(limit: int = Query(10, ge=1, le=50), db: Session = Depends(get_db)):
...
async def get_popular_products(limit: int = Query(10, ge=1, le=50), db: Session = Depends(get_db)):
...
async def get_user_details(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        location_scans = db.execute(text("""
...
async def cleanup_old_data(
    days_to_keep: int = Query(365, ge=30, le=1095),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        return {
            ...
            "total_deleted": deleted_sessions + deleted_location_scans + deleted_price_scans
        }
