...
def get_or_create_user(user_id: str, db: Session):
    ...
    db.execute(
        ...
        {"user_id": user_id, "email": f"{user_id}@temp.local", "consent": True},
    )
    db.commit()
    return user_id
...
async def track_location_scan(
    ...
    try:
        ...
        background_tasks.add_task(
            calculate_time_spent, user_id, request.monument_name, db
        )
...
async def track_price_scan(
    ...
    try:
        ...
        print(
            f"💰 Price Scan Stored: {request.product_name} - {request.detected_price} MAD by {user_id}"
        )
...
async def track_price_rating(
    ...
    try:
        ...
        print(
            f"⭐ Price Rating Stored: {request.price_fairness_rating} stars for scan {request.scan_id}"
        )
...
async def get_user_journey(user_id: str, db: Session = Depends(get_db)):
    """Retrieve aggregated journey summary"""
    try:
        # Get user analytics from the view
        result = db.execute(
            text("SELECT * FROM user_analytics WHERE user_id = :user_id"),
            {"user_id": user_id},
        )
        ...
        journey_data = {
            ...
            "created_at": (
                user_dict["created_at"].isoformat() if user_dict["created_at"] else None
            ),
            "last_active": (
                user_dict["last_active"].isoformat()
            ...
            "avg_location_rating": (
                float(user_dict["avg_location_rating"])
                if user_dict["avg_location_rating"]
                else None
            ),
            "avg_price_rating": (
                float(user_dict["avg_price_rating"])
                if user_dict["avg_price_rating"]
                else None
            ),
            "avg_hotel_rating": (
                float(user_dict["avg_hotel_rating"])
                if user_dict["avg_hotel_rating"]
                else None
            ),
            "most_searched_product_category": (
                most_searched_category[0] if most_searched_category else None
            ),
            "most_visited_monument_type": (
                most_visited_monument[0] if most_visited_monument else None
            ),
            "total_scans": (user_dict["location_scans"] or 0)
            + (user_dict["price_scans"] or 0),
        }

        print(
            f"📊 Journey Data Retrieved for {user_id}: {journey_data['total_scans']} total scans"
        )
