...
async def start_journey(request: JourneyStartRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        user_profile = db.query(UserProfile).filter(
            UserProfile.session_id == request.user_id
        ).first()
...
async def log_location_visit(request: LocationVisitRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Update journey map
        journey = db.query(UserJourneyMap).filter(
        ...
        raise HTTPException(status_code=500, detail=f"Failed to log location visit: {str(e)}")
...
async def end_journey(request: JourneyEndRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if journey:
            ...
            # Update notes
            journey.notes = (journey.notes or "") + f" | Journey ended after {request.total_duration_seconds}s with {request.locations_visited} locations visited"
...
async def get_journey_history(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        # Get user profile
        user_profile = db.query(UserProfile).filter(
            UserProfile.session_id == user_id
        ).first()
        ...
        # Get all journeys
        journeys = db.query(UserJourneyMap).filter(
            UserJourneyMap.user_profile_id == user_profile.id
        ).order_by(UserJourneyMap.journey_date.desc()).all()

        journey_data = []
        for journey in journeys:
            journey_data.append({
                "journey_id": str(journey.id),
                "date": journey.journey_date.isoformat() if journey.journey_date else None,
        ...
        return {
            ...
            "user_stats": {
                ...
                "member_since": user_profile.created_at.isoformat() if user_profile.created_at else None
            }
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get journey history: {str(e)}")
