            ...
            with self.SessionLocal() as db:
                ...
                for scan in scans_to_update:
                    ...
                    next_scan = db.execute(
                        ...
                        LIMIT 1
                    """),
                        {"user_id": user_id, "current_timestamp": scan_timestamp},
                    ).fetchone()
                ...
                db.commit()
                logger.info(
                    f"Updated dwell times for {len(scans_to_update)} location scans"
                )
        ...
        try:
            with self.SessionLocal() as db:
                ...
                report["traveler_types"] = [
                    {"type": ttype[0], "count": ttype[1]} for ttype in traveler_types
                ]
    ...
    def cleanup_old_data(self, days_to_keep: int = 365):
        ...
        try:
            with self.SessionLocal() as db:
                ...
                logger.info(
                    f"Cleaned up old data: {deleted_sessions} sessions, {deleted_location_scans} location scans, {deleted_price_scans} price scans"
                )
