...
def check_dataset(images_root):
    ...
    for product_folder in product_folders:
        ...
        for label_file in labels:
            ...
            if not img_found:
                issues.append(
                    f"No image found for label {label_file.name} in {product_folder.name}"
                )
        ...
        for label_file in labels:
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if len(parts) < 5:
                        issues.append(
                            f"Invalid label format in {label_file.name} line {i+1}: {line}"
                        )
                    else:
                        ...
                        try:
                            ...
                            if not all(0 <= x <= 1 for x in coords):
                                issues.append(
                                    f"Invalid coordinates in {label_file.name} line {i+1}: {line}"
                                )
                        except ValueError:
                            issues.append(
                                f"Non-numeric coordinates in {label_file.name} line {i+1}: {line}"
                            )
