...
class SimpleYOLOTrainer:
    ...
    def collect_all_data(self):
        ...
        product_folders = [
            f
            for f in self.images_root.iterdir()
    ...
    def create_augmented_dataset(
        self, all_pairs, classes, train_split=0.8, augment_count=2
    ):
    ...
    def save_augmented_pair(self, img_file, label_file, class_id, split, name):
        ...
        with open(dst_label, "w") as f:
            for label in aug_labels:
                f.write(
                    f"{class_id} {label[0]:.6f} {label[1]:.6f} {label[2]:.6f} {label[3]:.6f}\n"
                )
