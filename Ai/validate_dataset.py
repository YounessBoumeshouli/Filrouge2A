...
def validate_dataset(images_root):
    ...
    label_folders = [f for f in images_root.iterdir() 
    ...
    for product_folder in product_folders:
                        ...
                        if line:
                            ...
                            if len(parts) >= 5:
                                ...
                                if all(0 <= x <= 1 for x in coords):
                                    ...
                                    invalid_labels.append(f"{label_file.name}: invalid coordinates")
                            else:
                                invalid_labels.append(f"{label_file.name}: insufficient data")
                    
                    if valid_lines > 0:
                        matched_pairs += 1
                    else:
                        invalid_labels.append(f"{label_file.name}: no valid annotations")
        ...
        if orphaned_images:
            ...
            if len(orphaned_images) <= 3:
                ...
                print(f"     - {orphaned_images[0]} (and {len(orphaned_images)-1} more)")
        ...
        if orphaned_labels:
            ...
            if len(orphaned_labels) <= 3:
                ...
                print(f"     - {orphaned_labels[0]} (and {len(orphaned_labels)-1} more)")
